...
void Agent::ReduireListeSuspects(char reponse, string input, string caracteristique) {
	...
	switch (reponse) {
		...
		case 's':
			...
			for (unsigned int i = 0; i < tableauSuspect.size(); i++) {
...
void Agent::Corriger() {
	...
	if (answer == 'o') {
		for (unsigned int i = 0; i < mystereGuess.size(); i++) {
...
bool Agent::Deviner() {
	...
	if (tableauSuspect.size() == 2) { // si il reste deux individus dans le tableau 
		...
		for (unsigned int i = 0; i < mystereVrai.size(); i++) {
...
void Agent::affichage() {
	...
	if (mystereNonDeviner.size() != 0) {
		for (unsigned int i = 0; i < mystereNonDeviner.size(); i++) {
